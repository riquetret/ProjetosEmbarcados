...
static bool iniciar_calibracao_adc(adc_unit_t unit, adc_channel_t channel, adc_atten_t atten, adc_cali_handle_t *out_handle) {
    ...
    *out_handle = handle;
    if (ret == ESP_OK) ESP_LOGI(TAG, "Calibração ADC: Sucesso");
    else ESP_LOGW(TAG, "Calibração ADC: Não suportada, usando valores brutos");
    return calibrated;
}
...
void configurar_adc() {
    ...
    do_calibration = iniciar_calibracao_adc(ADC_UNIT_1, ADC_CHANNEL_3, ADC_ATTEN_DB_12, &adc1_cali_handle);
}
...
void configurar_timer(gptimer_handle_t *timer_ret) {
    gptimer_config_t timer_config = {
        ...
        .resolution_hz = 1E6, // 10us por tick
    };
...
void app_main(void) {
    ...
    while (1) {
        ...
        //     //ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, ADC_CHANNEL_3, &valor_bruto));
            
        //     // float x_n = 0.0f;
        //     // if (do_calibration) {
        //     //     adc_cali_raw_to_voltage(adc1_cali_handle, valor_bruto, &tensao_mv);
        //     //     x_n = 12.0f - ((float)tensao_mv / 1000.0f);
        //     // } else {
        //     //     x_n = 12.0f - ((float)valor_bruto * 3.3f / 4095.0f);
        //     // }
